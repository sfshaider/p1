...
# visa net DirectLink-securenet version 1.7
...
if ( ( -e "/home/p/pay1/batchfiles/$devprod/stopgenfiles.txt" ) || ( -e "/home/p/pay1/batchfiles/$devprod/securenet/stopgenfiles.txt" ) ) {
  exit;
}

$cnt = `ps -ef | grep -v grep | grep -v vim | grep perl | grep -c 'securenet/genfiles.pl $mygroup'`;
if ( $cnt > 1 ) {
  ...
  print MAILERR "Subject: securenet - genfiles already running\n";
  print MAILERR "\n";
...
chop $machine;
open( outfile, ">/home/p/pay1/batchfiles/$devprod/securenet/pid$mygroup.txt" );
...
open( infile, "/home/p/pay1/batchfiles/$devprod/securenet/pid$mygroup.txt" );
$chkline = <infile>;
chop $chkline;
close(infile);
...
if ( $pidline ne $chkline ) {
  ...
  print MAILERR "From: dcprice\@plugnpay.com\n";
  print MAILERR "Subject: securenet - dup genfiles\n";
  print MAILERR "\n";
...
$time = time();
( $d1, $d2, $d3, $d4, $d5, $d6, $d7, $d8, $d9, $modtime ) = stat "/home/p/pay1/batchfiles/$devprod/securenet/genfiles$mygroup.txt";

$delta = $time - $modtime;

if ( $delta < ( 3600 * 12 ) ) {
  umask 0033;
  open( checkin, "/home/p/pay1/batchfiles/$devprod/securenet/genfiles$mygroup.txt" );
  $checkuser = <checkin>;
  chop $checkuser;
  close(checkin);
}
...
#$checkstring = "and t.username='testsec'";
#$checkstring = "and t.username in ('membergone1')";
...
if ( !-e "/home/p/pay1/batchfiles/$devprod/securenet/$fileyearonly" ) {
  print "creating $fileyearonly\n";
  system("mkdir /home/p/pay1/batchfiles/$devprod/securenet/$fileyearonly");
  chmod( 0700, "/home/p/pay1/batchfiles/$devprod/securenet/$fileyearonly" );
}
if ( !-e "/home/p/pay1/batchfiles/$devprod/securenet/$filemonth" ) {
  print "creating $filemonth\n";
  system("mkdir /home/p/pay1/batchfiles/$devprod/securenet/$filemonth");
  chmod( 0700, "/home/p/pay1/batchfiles/$devprod/securenet/$filemonth" );
}
if ( !-e "/home/p/pay1/batchfiles/$devprod/securenet/$fileyear" ) {
  print "creating $fileyear\n";
  system("mkdir /home/p/pay1/batchfiles/$devprod/securenet/$fileyear");
  chmod( 0700, "/home/p/pay1/batchfiles/$devprod/securenet/$fileyear" );
}
if ( !-e "/home/p/pay1/batchfiles/$devprod/securenet/$fileyear" ) {
  open( MAILERR, "| /usr/lib/sendmail -t" );
  print MAILERR "To: cprice\@plugnpay.com\n";
  print MAILERR "From: dcprice\@plugnpay.com\n";
  print MAILERR "Subject: securenet - FAILURE\n";
  print MAILERR "\n";
  print MAILERR "Couldn't create directory $devprod/securenet/$fileyear.\n\n";
  close MAILERR;
  exit;
}
...
unlink "/home/p/pay1/batchfiles/$devprod/securenet/batchfile.txt";
...
sub pidcheck {
  ...
  if ( $pidline ne $chkline ) {
    umask 0077;
    open( logfile, ">>/home/p/pay1/batchfiles/$devprod/securenet/$fileyear/$username$time$pid.txt" );
...
sub processbatch {
  ...
  umask 0033;
  open( batchfile, ">/home/p/pay1/batchfiles/$devprod/securenet/batchfile.txt" );
  ...
  if ( $usercountarray{$username} > 2000 ) {
    $batchcntuser = 500;    # securenet has trouble doing more than 500 records in a batch
  ...
  local $sthcust = $dbh->prepare(
    qq{
		...
		v.agentbank,v.agentchain,v.storenum,v.categorycode,v.bin,v.terminalnum,v.industrycode,v.batchtime,v.capabilities
        from customers c, securenet v
        where c.username='$username'
        and c.processor='securenet'
  ...
  ( $merchant_id, $terminal_id, $proc_type, $company,    $address,  $city,         $state, $zip,         $tel,          $status,     $currency,
    $switchtime,  $features,    $agentbank, $agentchain, $storenum, $categorycode, $bin,   $terminalnum, $industrycode, $batchgroup, $capabilities
  )
    = $sthcust->fetchrow;
  $sthcust->finish;
  ...
  #if (($mygroup eq "3") && ($batchgroup ne "3")) {
  #  return;
  #}
  ...
  while ( $redobatch == 1 ) {
    ...
    foreach $orderid ( sort keys %orderidarray ) {
      ...
      $sthtrans2 = $dbh2->prepare(
        qq{
            select lastop,trans_date,lastoptime,enccardnumber,length,card_exp,amount,auth_code,avs,transflags,refnumber,lastopstatus
            from operation_log
      ...
      ( $operation, $trans_date, $trans_time, $enccardnumber, $enclength, $exp, $amount, $auth_code, $avs_code, $transflags, $refnumber, $finalstatus ) = $sthtrans2->fetchrow;
      $sthtrans2->finish;
      ...
      $enccardnumber = &smpsutils::getcardnumber( $username, $orderid, "securenet", $enccardnumber );
...
sub batchheader {
  ...
  if ( $industrycode !~ /^(retail|grocery|restaurant)$/ ) {
    $bh[5] = 'H@@@B';          # record type (5a)
  } else {
    $bh[5] = 'H@@@@';          # record type (5a)
  }
  ...
  &sendrecord($message);
  $respstr = unpack "H*", $response;
  print "respstr: $respstr\n";
  if ( ( length($response) != 1 ) || ( $response !~ /^(\x05|\x06|\x07)$/ ) ) {
    $errorcnt{$username}++;
    &error("header");
  }
...
sub batchdetail {
  ...
  if ( ( $operation eq "postauth" ) && ( ( length($auth_code) == 8 ) || ( length($auth_code) == 20 ) ) ) {
    ...
    my $sth = $dbh->prepare(
      qq{
        update securenet set transseqnum=?
        where username='$username'
        }
      )
      or &miscutils::errmail( __LINE__, __FILE__, "Can't prepare: $DBI::errstr", %datainfo );
  ...
  if ( $transflags =~ /level3/ ) {
    ...
    $merchvatregnum = substr( $merchvatregnum . " " x 20, 0,   20 );
    $custvatregnum  = substr( $custvatregnum . " " x 13,  0,   13 );
    $commidtycode   = substr( $commidtycode . " " x 4,    0,   4 );
    $discount       = substr( "0" x 12 . $discount,       -12, 12 );
    $freight        = substr( "0" x 12 . $freight,        -12, 12 );
    $duty           = substr( "0" x 12 . $duty,           -12, 12 );
    $destzip        = substr( $zip . " " x 10,            0,   10 );
    $srczip         = substr( $merchzip . " " x 10,       0,   10 );
    $country        = substr( $country . " " x 3,         0,   3 );     # USA or 840 ?
    $vatrefnum      = substr( $vatrefnum . " " x 15,      0,   15 );    # unique
    $orderdate      = substr( $trans_date,                2,   6 );     # YYMMDD
    $vattax         = substr( "0" x 12 . $vattax,         -12, 12 );
    $vattaxrate     = substr( "0" x 4 . $vattaxrate,      -4,  4 );
    $itemcount      = substr( "0" x 3 . $itemcount,       -3,  3 );

    $alttaxamtind = substr( $alttaxamtind . " ",  0,  1 );
    $alttaxamt    = substr( "0" x 9 . $alttaxamt, -9, 9 );

    if ( $card_type eq "vi" ) {
      $group[26] = "$merchvatregnum$custvatregnum$commidtycode$discount$freight$duty$destzip$srczip$country$vatrefnum$orderdate";
      $group[26] = $group[26] . "$vattax$vattaxrate$itemcount";
    } elsif ( $card_type eq "mc" ) {
      $group[26] = "$freight$duty$destzip$srczip$country$alttaxamtind$alttaxamt$itemcount";
    }
  }
  ...
  if ( $transflags =~ /level3/ ) {
    ...
    while ( $sthdetails->fetch ) {
      ...
      $bd[3] = '.';                # message delimiter (1a)
      $bd[4] = 'Z';                # X.25 Routing ID (1a)
      $bd[5] = 'L@@@@';            # record type (5a)

      $bd[6] = 'VISA';             # line item card type
      ...
      $bd[7]  = "$item";          # item commodity code (12a)
      $bd[8]  = "$descr";         # item descriptor (35a)
      $bd[9]  = "$prod";          # product code (12a)
      $bd[10] = "$quantity";      # quantity (12a)
      $bd[11] = "$unit";          # unit of measure (12a)
      $bd[12] = "$unitcost";      # cost (12a)
      $bd[13] = "$vattax";        # vattax (12a)
      $bd[14] = "$vattaxrate";    # vattaxrate (4a)
      $bd[15] = "$discount";      # quantity (12a)
      $bd[16] = "$total";         # total (12a)
...
sub sendrecord {
  ...
  print "$mytime send: $rlen $message2\n";
  close(logfile);
...
sub sslsend {
  ...
  if ( $respcode eq "GB" ) {
    ...
    print MAILERR "Subject: securenet - RB INV DATA\n";
    print MAILERR "\n";
    ...
    print MAILERR "To: cprice\@plugnpay.com\n";
    print MAILERR "From: dprice\@plugnpay.com\n";
    print MAILERR "Subject: securenet - duplicate batch\n";
    print MAILERR "\n";
    print MAILERR "username: $username\n";
    ...
    print MAILERR "To: cprice\@plugnpay.com\n";
    print MAILERR "From: dprice\@plugnpay.com\n";
    print MAILERR "Subject: securenet - vnetssl no response\n";
    print MAILERR "\n";
    print MAILERR "username: $username\n";
    print MAILERR "result: $time\n";
    print MAILERR "file: $username$time$pid.txt\n\n";
    ...
    print MAILERR "To: cprice\@plugnpay.com\n";
    print MAILERR "From: dprice\@plugnpay.com\n";
    print MAILERR "Subject: securenet - unkown error\n";
    print MAILERR "\n";
    print MAILERR "username: $username\n";
...
sub socketwrite {
  ...
  my $mytime = gmtime( time() );
  open( logfile, ">>/home/p/pay1/batchfiles/$devprod/securenet/$fileyear/$username$time$pid.txt" );
  print logfile "$mytime recv: $rlen $message2\n";
...
sub vnetsslopen2 {
  ...
  $site      = "ssl5.securenet.com";    # production
  $dest_serv = $site;
  $port      = "5081";
...
sub vnetsslsend2 {
  ...
  my $respdec = "";
  my $rlen    = length($message);
  for ( my $ii = 0 ; $ii < $rlen ; $ii++ ) {
    $resp1   = substr( $message, $ii, 1 );
    $newresp = $resp1 & "\x7f";
    $respdec = $respdec . $newresp;
  }
  $respdec =~ s/\x1c/\[1c\]/g;
  ...
  $respdec =~ s/\x17/\[ETB\]/g;
  $respdec =~ s/\x03/\[ETX\]/g;
  print "message: $respdec\n";

  # Exchange data
  $res = Net::SSLeay::ssl_write_all( $ssl, $message );    # Perl knows how long $msg is
  ...
  while ( $count && select( $rout = $rin, undef, undef, 60.0 ) ) {
    ...
    $got    = Net::SSLeay::read($ssl);    # Perl returns undef on failure
                                          #umask 0011;
                                          #open(tmpfile,">>/home/p/pay1/batchfiles/$devprod/securenet/bserverlogmsg.txt");
