...
if ( ( -e "/home/p/pay1/batchfiles/stopgenfiles.txt" ) || ( -e "/home/p/pay1/batchfiles/fdmsemv/stopgenfiles.txt" ) ) {
  exit;
}

$cnt = `ps -ef | grep -v grep | grep -v vim | grep perl | grep -c 'fdmsemv/genfiles.pl $group'`;
if ( $cnt > 1 ) {
...
chop $machine;
open( outfile, ">/home/p/pay1/batchfiles/fdmsemv/pid$group.txt" );
...
open( infile, "/home/p/pay1/batchfiles/fdmsemv/pid$group.txt" );
$chkline = <infile>;
chop $chkline;
close(infile);
...
if ( $pidline ne $chkline ) {
  ...
  print MAILERR "Subject: fdmsemv - dup genfiles\n";
  print MAILERR "\n";
...
$checkstring = " and t.username='testfdmsemv'";
...
if ( !-e "/home/p/pay1/batchfiles/fdmsemv/logs/$fileyearonly" ) {
  print "creating $fileyearonly\n";
  system("mkdir /home/p/pay1/batchfiles/fdmsemv/logs/$fileyearonly");
  chmod( 0700, "/home/p/pay1/batchfiles/fdmsemv/logs/$fileyearonly" );
}
if ( !-e "/home/p/pay1/batchfiles/fdmsemv/logs/$filemonth" ) {
  print "creating $filemonth\n";
  system("mkdir /home/p/pay1/batchfiles/fdmsemv/logs/$filemonth");
  chmod( 0700, "/home/p/pay1/batchfiles/fdmsemv/logs/$filemonth" );
}
if ( !-e "/home/p/pay1/batchfiles/fdmsemv/logs/$fileyear" ) {
  print "creating $fileyear\n";
  system("mkdir /home/p/pay1/batchfiles/fdmsemv/logs/$fileyear");
  chmod( 0700, "/home/p/pay1/batchfiles/fdmsemv/logs/$fileyear" );
}
if ( !-e "/home/p/pay1/batchfiles/fdmsemv/logs/$fileyear" ) {
  open( MAILERR, "| /usr/lib/sendmail -t" );
  print MAILERR "To: cprice\@plugnpay.com\n";
  print MAILERR "From: dcprice\@plugnpay.com\n";
  print MAILERR "Subject: fdmsemv - FAILURE\n";
  print MAILERR "\n";
  print MAILERR "Couldn't create directory fdmsemv/logs/$fileyear.\n\n";
  close MAILERR;
  exit;
}
...
foreach $key ( sort keys %userbankarray ) {
  ...
  if ( ( -e "/home/p/pay1/batchfiles/stopgenfiles.txt" ) || ( -e "/home/p/pay1/batchfiles/fdmsemv/stopgenfiles.txt" ) ) {
    ...
    print "stopgenfiles\n";
    close(logfile);
    unlink "/home/p/pay1/batchfiles/fdmsemv/batchfile.txt";
    last;
  }

  umask 0033;
  open( batchfile, ">/home/p/pay1/batchfiles/fdmsemv/genfiles$group.txt" );
  print batchfile "$username\n";
  close(batchfile);

  print "bbbb $username  $banknum\n";
  umask 0033;
  open( batchfile, ">/home/p/pay1/batchfiles/fdmsemv/batchfile.txt" );
  print batchfile "$username\n";
  close(batchfile);
  ...
  local $sthinfo = $dbh->prepare(
    qq{
        select industrycode,fedtaxid,vattaxid,categorycode,chargedescr,batchtime
        from fdmsemv
        where username='$username'
        }
    )
  ...
  ( $industrycode, $fedtaxid, $vattaxid, $categorycode, $chargedescr, $batchgroup ) = $sthinfo->fetchrow;
  $sthinfo->finish;
...
$mytime = gmtime( time() );
umask 0077;
open( outfile, ">>/home/p/pay1/batchfiles/fdmsemv/ftplog.txt" );
print outfile "\n\n$mytime\n";
close(outfile);
...
sub batchdetail {
  ...
  if ( ( $card_type eq "vi" ) && ( $origoperation ne "forceauth" ) && ( $operation ne "return" ) ) {
    ...
    $authrespcode = substr( $auth_code, 335, 2 );
...
sub batchheader {
  ...
  if ( ( $marketdata eq "" ) || ( $marketdata eq " " x 38 ) ) {
    ...
    $bh[2] = " ";           # filler (1a)
    $phone = substr( $marketdata, 118, 13 );
    $phone =~ tr/a-z/A-Z/;
...
sub fileheader {
  ...
  local $sthinfo = $dbh->prepare(
    qq{
        update fdmsemv set filenum=?,batchdate=?
	where username='fdmsemv'
        }
    )
    or &miscutils::errmail( __LINE__, __FILE__, "Can't prepare: $DBI::errstr", %datainfo );
  ...
  umask 0077;
  open( outfile,  ">/home/p/pay1/batchfiles/fdmsemv/logs/$fileyear/$filename" );
  open( outfile2, ">/home/p/pay1/batchfiles/fdmsemv/logs/$fileyear/$filename.txt" );
