...
chop $machine;
open( outfile, ">/home/p/pay1/batchfiles/$devprod/planetpay/pid.txt" );
...
open( infile, "/home/p/pay1/batchfiles/$devprod/planetpay/pid.txt" );
$chkline = <infile>;
chop $chkline;
close(infile);
...
open( checkin, "/home/p/pay1/batchfiles/$devprod/planetpay/genfiles.txt" );
$checkuser = <checkin>;
chop $checkuser;
close(checkin);
...
#$checkstring = "and t.username='testplanet'";
...
if ( !-e "/home/p/pay1/batchfiles/$devprod/planetpay/$fileyearonly" ) {
  print "creating $fileyearonly\n";
  system("mkdir /home/p/pay1/batchfiles/$devprod/planetpay/$fileyearonly");
  chmod( 0700, "/home/p/pay1/batchfiles/$devprod/planetpay/$fileyearonly" );
}
if ( !-e "/home/p/pay1/batchfiles/$devprod/planetpay/$filemonth" ) {
  print "creating $filemonth\n";
  system("mkdir /home/p/pay1/batchfiles/$devprod/planetpay/$filemonth");
  chmod( 0700, "/home/p/pay1/batchfiles/$devprod/planetpay/$filemonth" );
}
if ( !-e "/home/p/pay1/batchfiles/$devprod/planetpay/$fileyear" ) {
  print "creating $fileyear\n";
  system("mkdir /home/p/pay1/batchfiles/$devprod/planetpay/$fileyear");
  chmod( 0700, "/home/p/pay1/batchfiles/$devprod/planetpay/$fileyear" );
}
if ( !-e "/home/p/pay1/batchfiles/$devprod/planetpay/$fileyear" ) {
  open( MAILERR, "| /usr/lib/sendmail -t" );
  print MAILERR "To: cprice\@plugnpay.com\n";
...
foreach $username ( sort @userarray ) {
  if ( -e "/home/p/pay1/batchfiles/$devprod/stopgenfiles.txt" ) {
    unlink "/home/p/pay1/batchfiles/$devprod/planetpay/batchfile.txt";
    last;
  }

  umask 0033;
  open( batchfile, ">/home/p/pay1/batchfiles/$devprod/planetpay/batchfile.txt" );
  ...
  umask 0077;
  open( logfile, ">>/home/p/pay1/batchfiles/$devprod/planetpay/$fileyear/$username$time$pid.txt" );
  print "$username\n";
  ...
  ( $merchant_id, $terminal_id, $proc_type, $company, $address, $city, $state, $zip, $tel, $status, $currency ) = $sthcust->fetchrow;
  $sthcust->finish;

  my $sthcust2 = $dbh->prepare(
    qq{
    ...
    or &miscutils::errmaildie( __LINE__, __FILE__, "Can't prepare: $DBI::errstr", %datainfo );
  $sthcust2->execute or &miscutils::errmaildie( __LINE__, __FILE__, "Can't execute: $DBI::errstr", %datainfo );
  ( $terminalnum, $industrycode ) = $sthcust2->fetchrow;
  $sthcust2->finish;
  ...
  while ( $sthtrans->fetch ) {
    ...
    &batchdetail();
    $response = &socketwritessl( "secure.planetpayment.net", "443", "/WebService/PlugNPay4Km5/PlanetPaymentWS/?wsdl", "$message" );    # production
...
sub endbatch {
  ...
  open( tmpfile, ">>/home/p/pay1/batchfiles/$devprod/planetpay/scriptresults2.txt" );
  print tmpfile "$tempamount      $oid    $refnumber      $operation\n";
  close(tmpfile);
  ...
  print "pass: $planetpay::pass\n";
  print "err_msg: $planetpay::err_msg\n";
  print "refnum: $planetpay::refnum\n";
  ...
  if ( $respcode eq "00" ) {
    ...
    my $sthpass = $dbh2->prepare(
      ...
      or &miscutils::errmail( __LINE__, __FILE__, "Can't prepare: $DBI::errstr", %datainfo );
    $sthpass->execute( "$time", "$auth_code", "$refnumber" ) or &miscutils::errmail( __LINE__, __FILE__, "Can't execute: $DBI::errstr", %datainfo );
    $sthpass->finish;
    ...
    my $sthop = $dbh2->prepare(
      ...
      or &miscutils::errmail( __LINE__, __FILE__, "Can't prepare: $DBI::errstr", %datainfo );
    $sthop->execute( "$time", "$auth_code", "$refnumber" ) or &miscutils::errmail( __LINE__, __FILE__, "Can't execute: $DBI::errstr", %datainfo );
  } elsif ( $err_msg ne "" ) {
...
sub batchdetail {
  ...
  if ( $operation eq "postauth" ) {
    ...
    $currency2 = $isotables::currencyUSD840{$currency2};
    $amount = sprintf( "%d", ( $amount * ( 10**$exponent ) ) + .0001 );
    print "currency: $currency  $currency2\n";
...
sub sendbatchclose {
  ...
  #$response = &socketwritessl("uat.planetpayment.net","443","/QAPlugnPayWS/?wsdl","$message");       # test
  $response = &socketwritessl( "secure.planetpayment.net", "443", "/WebService/PlugNPay4Km5/PlanetPaymentWS/?wsdl", "$message" );    # production
  print "$response\n";
  ...
  open( tmpfile, ">>/home/p/pay1/batchfiles/$devprod/planetpay/scriptresults2.txt" );
  print tmpfile "       	$oid	$refnum	batch close\n";
  close(tmpfile);

  umask 0077;
  open( logfile, ">>/home/p/pay1/batchfiles/$devprod/planetpay/$fileyear/$username$time$pid.txt" );
  print logfile "sendclose:\n";
  print logfile "batchnum: $batchnum\n";
  print logfile "status: $status\n";
  print logfile "respcode: $respcode\n";
  print logfile "errmsg: $errmsg\n";
  print logfile "refnum: $refnum\n";
  close(logfile);
}
...
sub sendbatchstatus {
  ...
  print "batchsalesamt: $batchsalesamt    $hostsalesamt\n";
  print "batchsalescnt: $batchsalescnt    $hostsalescnt\n";
  print "batchretamt: $batchretamt    $hostrefundsamt\n";
  print "batchretcnt: $batchretcnt    $hostrefundscnt\n";

  print "reversalamt: xxxx    $hostreversalamt\n";
  print "reversalcnt: xxxx    $hostreversalcnt\n";

  print "batchnum: $batchnum    $batchnumber\n";
  print "respcode: $respcode\n";
  print "errmsg: $errmsg\n";
  print "status: $status\n";
  print "refnum: $refnum\n";
  ...
  umask 0077;
  open( logfile, ">>/home/p/pay1/batchfiles/$devprod/planetpay/$fileyear/$username$time$pid.txt" );
  print logfile "sendinquiry:\n";
  print logfile "batchsalesamt: $batchsalesamt    $hostsalesamt\n";
  print logfile "batchsalescnt: $batchsalescnt    $hostsalescnt\n";
  print logfile "batchretamt: $batchretamt    $hostrefundsamt\n";
  print logfile "batchretcnt: $batchretcnt    $hostrefundscnt\n";

  print logfile "reversalamt: xxxx    $hostreversalamt\n";
  print logfile "reversalcnt: xxxx    $hostreversalcnt\n";

  print logfile "batchnum: $batchnum    $batchnumber\n";
  print logfile "respcode: $respcode\n";
  print logfile "errmsg: $errmsg\n";
  print logfile "status: $status\n";
  print logfile "refnum: $refnum\n";
...
sub sendbatchstatusdetail {
  ...
  foreach my $line (@lines) {
    ...
    print "type: $type  amount: $amount  currency: $currency  refnum: $refnum  oid: $oid\n";
    print logfile "type: $type  amount: $amount  currency: $currency  refnum: $refnum  oid: $oid\n";
  ...
  print logfile "\n";
  print logfile "The following orderids are on planet's list, but not ours:\n";
  foreach $oid ( sort keys %inplanetarray ) {
    print logfile "$oid\n";
  }
  print logfile "\n";

  print logfile "The following orderids are on our list, but not planet's:\n";
...
sub socketwritessl {
  my ( $host, $port, $path, $msg, $notSSL ) = @_;
  ...
  my $len      = length($msg);
  my $postLink = $protocol . "://" . $host . ":$port" . $path;
  ...
  #my $rl = new PlugNPay::ResponseLink('processor_planet',$postLink,$msg,'post','meta');
  my $rl = new PlugNPay::ResponseLink( 'planet', $postLink, $msg, 'post', 'meta' );
  $rl->setRequestContentType('text/xml');
  $rl->addRequestHeader( 'Host',           "$host:$port" );
  $rl->addRequestHeader( 'Accept',         "*/*" );
  $rl->addRequestHeader( 'SOAPAction',     "http://www.planetpayment.net/ITransaction/$tcode" );
  $rl->addRequestHeader( 'Content-Length', $len );

  print Dumper $rl->showRequest();
...
sub serverLogMsg {
  my ( $req, $direction, $datainfo ) = @_;
  my ($logFile);

  my $pathToLog = "/home/pay1/batchfiles/$devprod/planetpay";

  if ( $username =~ /^(testplanetpay|planetpaydem)/ ) {
    $logFile = "serverlogmsgtest.txt";
  } else {
    $logFile = "serverlogmsg.txt";
  }
  my $mytime     = gmtime( time() );
  my $chkmessage = $req;
  open( LOGFILE, ">>$pathToLog/$logFile" );
  print LOGFILE "$username $datainfo{'order-id'}\n\n";
  print LOGFILE "$mytime $direction: $chkmessage\n\n";
  close(LOGFILE);
