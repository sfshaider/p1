...
$cnt = `ps -ef | grep -v grep | grep -v vim | grep perl | grep -c 'fdmsintl/genfilesus.pl $group'`;
if ( $cnt > 1 ) {
  my $printstr = "genfilesus.pl already running, exiting...\n";
  &procutils::filewrite( "$username", "fdmsintl", "/home/pay1/batchfiles/devlogs/fdmsintl", "miscdebug.txt", "append", "misc", $printstr );
  exit;
}
...
if ( $pidline ne $chkline ) {
  my $printstr = "genfilesus.pl $group already running, pid alterred by another program, exiting...\n";
  $printstr .= "$pidline\n";
  $printstr .= "$chkline\n";
  &procutils::filewrite( "$username", "fdmsintl", "/home/pay1/batchfiles/devlogs/fdmsintl", "miscdebug.txt", "append", "misc", $printstr );
  ...
  print MAILERR "\n";
  print MAILERR "genfilesus.pl $group already running, pid alterred by another program, exiting...\n";
...
my $dbquerystr = <<"dbEOM";
        ...
        and o.processor in ('fdmsintlus','fdmsintl')
        group by t.username
dbEOM
...
dbEOM
  my @dbvalues = ($user);
  my ( $currency, $chkprocessor ) = &procutils::dbread( $username, $orderid, "pnpmisc", $dbquerystr, @dbvalues );

  if ( $currency eq "" ) {
    $currency = "usd";
  }

  if ( $chkprocessor ne "fdmsintlus" ) {
    next;
  }
...
dbEOM
    ...
    if ( ( $card_type =~ /dc|jc/ ) || ( $cardnumber =~ /^8/ ) ) {
      $card_type = "ds";
    }
    ...
    $bd[6]    = $authcode;                                 # authorization code
                                                           ...
                                                           #}
    if ( ( ( $currency eq "usd" ) && ( $card_type eq "vi" ) && ( $transflags =~ /recur/ ) && ( $transflags !~ /init/ ) )
      || ( ( $currency eq "usd" ) && ( $card_type eq "vi" ) && ( $transflags =~ /recur/ ) && ( $transflags =~ /init/ ) && ( $transflags =~ /notcof/ ) && ( $transflags !~ /moto/ ) )
      || ( ( $currency eq "usd" ) && ( $card_type eq "mc" ) && ( $transflags =~ /recur|mit/ )     && ( $transflags !~ /xinit/ ) )
      || ( ( $currency eq "usd" ) && ( $card_type eq "ax" ) && ( $transflags =~ /recur/ )         && ( $transflags !~ /init/ ) )
      || ( ( $currency eq "usd" ) && ( $card_type eq "ds" ) && ( $transflags =~ /recur/ )         && ( $transflags !~ /init/ ) )
      || ( ( $currency ne "usd" ) && ( $card_type eq "vi" ) && ( $transflags =~ /recur|install/ ) && ( $transflags !~ /init/ ) )
      || ( ( $currency ne "usd" ) && ( $card_type eq "mc" ) && ( $transflags =~ /recur|mit/ )     && ( $transflags !~ /xinit/ ) )
      || ( ( $currency ne "usd" ) && ( $card_type eq "ax" ) && ( $transflags =~ /recur|init/ )    && ( $transflags !~ /init/ ) )
      || ( ( $currency ne "usd" ) && ( $card_type eq "ds" ) && ( $transflags =~ /recur/ )         && ( $transflags !~ /init/ ) ) ) {
    ...
    if ( ( $industrycode =~ /^(retail|restaurant)$/ ) && ( $transflags !~ /moto/ ) ) {
      ...
      $ecomind = " ";
    } elsif ( ( $card_type eq "ax" ) && ( $eciind =~ /(5|6|7)/ ) && ( ( $transflags !~ /recur|mit|xcit/ ) || ( $transflags =~ /init/ ) ) ) {
      $ecomind = $eciind;
    } elsif (
      ( $cardtype eq "V" )
      && ( ( $transflags !~ /moto|recur|deferred/ )
        || ( ( $transflags !~ /moto/ ) && ( $transflags =~ /xinit/ ) && ( $transflags =~ /recur/ ) ) )
      ) {
    ...
    } elsif ( ( $cardtype eq "M" ) && ( $transflags !~ /moto|recur|mit|xcit/ ) ) {
      $ecomind = "J";
    } else {
      $ecomind = " ";
    }
    ...
    $bd[11] = $tid;        # location/store # , pos 8,9,10 of mid (3a)
    $bd[12] = "      ";    # filler (6a)
    ...
    if ( ( $industrycode =~ /^(retail|restaurant)$/ ) && ( $transflags !~ /moto/ ) ) {
      ...
      $cardholderid = "4";
      $posentry     = "1";
      if ( ( ( $card_type =~ /(vi)/ ) && ( $transflags =~ /resub|recur|mit|cit|install/ ) && ( $transflags !~ /init|notcof/ ) )
        || ( ( $card_type =~ /(mc)/ ) && ( $transflags =~ /recur|mit|cit|install/ ) && ( $transflags !~ /init|notcof/ ) ) ) {
        $posentry = "U";
      }
    }
    ...
    if ( ( $operation eq "postauth" ) && ( $card_type eq "ax" ) && ( $origoperation ne "forceauth" ) ) {
      ...
      $bd[5] = "0" x 4;         # reserved (4a)
      $bd[6] = " " x 3;         # filler (3a)

      ( $origcurr, $origamt ) = split( / /, $origamount, 2 );
      $origamt = sprintf( "%d", ( $origamt * 100 ) + .0001 );
      $origamt = substr( "0" x 13 . $origamt, -13, 13 );
      $bd[7] = $origamt;        # amex auth amount (13)

      $bd[8] = " " x 26;        # filler (26a)
    ...
    if ( ( $operation eq "postauth" ) && ( $card_type eq "ax" ) && ( $gratuity > 0 ) ) {
      $batchreccnt++;
      $filereccnt++;

      @bd    = ();
      $bd[0] = $tcode;    # tran code (2n) 06 = sale, 07 = return
      $bd[1] = "R";       # addendum record 1 (2n)

      my $foodbevamt = $transamt - $gratuity;
      $foodbevamt = substr( "00000000" . $foodbevamt, -8, 8 );
      $bd[2] = $foodbevamt;    # total food/beverage (8n)
      $bd[3] = "0" x 8;        # total beverage (8n)
      $gratuity = substr( "0" x 8 . $gratuity, -8, 8 );
      $bd[4] = $gratuity;      # waiter gratuity (8n)
      $bd[5] = "0" x 8;        # captain gratuity (8n)
      $bd[6] = " " x 45;       # filler (45)
    ...
    # clear spec only
    if ( ( $iabspecflag == 0 ) && ( $operation eq "postauth" ) && ( $card_type eq "ds" ) && ( $gratuity > 0 ) ) {
      $batchreccnt++;
      $filereccnt++;

      @bd    = ();
      $bd[0] = $tcode;    # tran code (2n) 06 = sale, 07 = return
      $bd[1] = "<F";      # addendum record 1 (2n)

      $gratuity = substr( "0" x 12 . $gratuity, -12, 12 );
      $bd[2] = $gratuity;    # gratuity (12a)
      $bd[3] = " " x 64;     # filler (64a)
    ...
    if (
      ...
      || ( ( $currency ne "xusd" ) && ( $operation eq "return" ) && ( $origcardtype =~ /(ds|dc)/ ) && ( $transflags !~ /offline/ ) && ( $transid ne "" ) )
      || ( ( $currency ne "usd" )
        && ( $operation eq "postauth" )
        && ( $origcardtype =~ /(ds|dc)/ )
        && ( $origoperation ne "forceauth" )
    ...
    # vi card on file
    if ( ( $card_type eq "vi" ) && ( $transflags =~ /(init|recur|install|mit|cit|reauth|incr|resub|delay|noshow)/ ) && ( $transflags !~ /notcof/ ) ) {
      $batchreccnt++;
      $filereccnt++;

      @bd    = ();
      $bd[0] = $tcode;      # tran code (2n) 06 = sale, 07 = return
      $bd[1] = "(6";        # addendum record 1 (2a)
      $bd[2] = "      ";    # transaction time (6n) HHMMSS
      my $tid = substr( "0" x 8 . $terminal_id, -8, 8 );
      $bd[3] = $tid;        # terminal id (8n)
      $bd[4] = "     ";     # visa checkout (5a)
      $bd[5] = " " x 16;    # filler (16a)
      ...
      $bd[6] = $posenv;     # visa pos environment (1a)
      $bd[7] = "  ";        # visa business application identifier (2a)
      $bd[8] = " ";         # filler (1a)
      $bd[9] = " " x 37;    # filler (37a)
    ...
    # installment not supported by FDMS
    ...
    my $cavv = substr( $auth_code, 364, 40 );
    $cavv =~ s/ //g;
    if ( ( $operation eq "postauth" ) && ( $card_type eq "mc" ) && ( $cavv ne "" ) ) {
      $batchreccnt++;
      $filereccnt++;

      @bd    = ();
      $bd[0] = "06";    # tran code (2n) 06 = sale
      $bd[1] = "=6";    # record type (2a)

      $cavv = substr( $cavv . " " x 32, 0, 32 );
      $bd[2] = $cavv;    # account authentication value aav (32a)

      my $xid = substr( $auth_code, 300, 64 );
      $xid = substr( $xid . " " x 36, 0, 36 );
      if ( $transflags !~ /3d2/ ) {
        $xid = " " x 36;
      }
      $bd[3] = $xid;     # directory servertransaction id (36a)

      my $protocol = "1";
      if ( $transflags =~ /3d2/ ) {
        $protocol = "2";
      }
      $bd[4] = $protocol;    # protocol (1a)
      $bd[5] = " " x 7;      # filler (7a)
...
# let genfiles.pl coordinate putfiles and getfiles
if (0) {
  if ( ( $filecount > 0 ) && ( $filecount < 80 ) ) {
    for ( $myi = 0 ; $myi <= ( $filecount + 3 ) ; $myi++ ) {
      umask 0077;
      $mytime     = gmtime( time() );
      $outfilestr = "";
...
sub fileheader {
  ...
  # RCDMPLG2	fh  00 PLUG...    Clear Spec
  # RCDMPLG3	fh  00 PLUG...    Clear Spec
  # RCDMPLGS	fh  00 +banknum   IAB spec
...
sub pidcheck {
  ...
  if ( $pidline ne $chkline ) {
    umask 0077;
    $logfilestr = "";
    $logfilestr .= "genfilesus.pl $group already running, pid alterred by another program, exiting...\n";
    $logfilestr .= "$pidline\n";
    $logfilestr .= "$chkline\n";
    &procutils::filewrite( "$username", "fdmsintl", "/home/pay1/batchfiles/$devprod/fdmsintl/$fileyear", "$username$time$pid.txt", "append", "", $logfilestr );
