...
if ( $test eq "yes" ) {
  ...
  &procutils::filewrite( "$username", "fdmsomaha", "/home/pay1/batchfiles/$devprod/fdmsomaha", "bserverlogmsg.txt", "append", "", $logfilestr );
...
while (1) {
  ...
  $outfilestr = "";
  $outfilestr .= "$temptime\n";
  &procutils::flagwrite( "$username", "fdmsomaha", "/home/pay1/batchfiles/$devprod/fdmsomaha", "baccesstime.txt", "write", "", $outfilestr );
...
dbEOM
    ...
    my $delta  = $now - $mytime;
    if ( $delta > 60 ) {
      &procutils::updateprocmsg( $processid, "fdmsob", "failure", "", "failure: message timeout" );
      next;
    }
...
dbEOM
    ...
    $cvv = substr( $message, 89, 4 );
    if ( $cvv =~ /^[0-9]{3} / ) {
      $messagestr = substr( $messagestr, 0, 89 ) . "xxx " . substr( $messagestr, 93 );
    } elsif ( $cvv =~ /^[0-9]{4}/ ) {
      $messagestr = substr( $messagestr, 0, 89 ) . "xxxx" . substr( $messagestr, 93 );
    }
  ...
  while ( $count && select( $rout = $rin, undef, undef, $mydelay ) ) {
    ...
    while ( ( $rlen >= $resplength ) && ( $rlen > 0 ) ) {
      ...
      if ( $nullresp eq $nullmessage ) {
        ...
        &procutils::filewrite( "$username", "fdmsomaha", "/home/pay1/batchfiles/$devprod/fdmsomaha", "bserverlogmsg.txt", "append", "", $tmpfilestr );
...
sub updatefdmsomaha {
  ...
  &procutils::updateprocmsg( $processid, "fdmsob", "success", "", "$response" );
