...
$cnt = `ps -ef | grep -v grep | grep -v vim | grep perl | grep -c 'telecheckftf/genfiles.pl'`;
if ( $cnt > 1 ) {
  ...
  print MAILERR "Subject: telecheckftf - genfiles already running\n";
  print MAILERR "\n";
...
$outfilestr .= "$pidline\n";
&procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/$devprod/telecheckftf", "pid.txt", "write", "", $outfilestr );

&miscutils::mysleep(2.0);

my $chkline = &procutils::fileread( "$username", "telecheckftf", "/home/pay1/batchfiles/$devprod/telecheckftf", "pid.txt" );
chop $chkline;
...
if ( $pidline ne $chkline ) {
  ...
  print MAILERR "From: dcprice\@plugnpay.com\n";
  print MAILERR "Subject: telecheckftf - dup genfiles\n";
  print MAILERR "\n";
...
#open(checkin,"/home/pay1/batchfiles/$devprod/telecheckftf/genfiles.txt");
#$checkuser = <checkin>;
#chop $checkuser;
#close(checkin);
...
#}
#$checkstring = "and t.username='jhewitt01'";
...
if ( !-e "/home/pay1/batchfiles/$devprod/telecheckftf/$fileyearonly" ) {
  my $printstr = "creating $fileyearonly\n";
  &procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/devlogs/telecheckftf", "miscdebug.txt", "append", "misc", $printstr );
  system("mkdir /home/pay1/batchfiles/$devprod/telecheckftf/$fileyearonly");
  chmod( 0700, "/home/pay1/batchfiles/$devprod/telecheckftf/$fileyearonly" );
}
if ( !-e "/home/pay1/batchfiles/$devprod/telecheckftf/$filemonth" ) {
  my $printstr = "creating $filemonth\n";
  &procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/devlogs/telecheckftf", "miscdebug.txt", "append", "misc", $printstr );
  system("mkdir /home/pay1/batchfiles/$devprod/telecheckftf/$filemonth");
  chmod( 0700, "/home/pay1/batchfiles/$devprod/telecheckftf/$filemonth" );
}
if ( !-e "/home/pay1/batchfiles/$devprod/telecheckftf/$fileyear" ) {
  my $printstr = "creating $fileyear\n";
  &procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/devlogs/telecheckftf", "miscdebug.txt", "append", "misc", $printstr );
  system("mkdir /home/pay1/batchfiles/$devprod/telecheckftf/$fileyear");
  chmod( 0700, "/home/pay1/batchfiles/$devprod/telecheckftf/$fileyear" );
}
if ( !-e "/home/pay1/batchfiles/$devprod/telecheckftf/$fileyear" ) {
  open( MAILERR, "| /usr/lib/sendmail -t" );
  print MAILERR "To: cprice\@plugnpay.com\n";
  print MAILERR "From: dcprice\@plugnpay.com\n";
  print MAILERR "Subject: telecheckftf - FAILURE\n";
  print MAILERR "\n";
  print MAILERR "Couldn't create directory telecheckftf/$fileyear.\n\n";
  close MAILERR;
  exit;
}
...
local %errcode = (
  ...
  "81", "Unable to locate original transaction (TCK Trace ID) Adjustment cannot be processed by telecheckftf",

  "OK",  "Inquiry (POS system) Packet was accepted and successfully processed by telecheckftf",
  "NAK", "Inquiry Packet was not successfully processed by telecheckftf (general error)",
  "49",  "Inquiry Packet was not successfully processed by telecheckftf (scheduled maintenance)",
  "97",  "Inquiry Packet was not successfully processed by telecheckftf (timeout)",
  "27",  "Inquiry Packet was not successfully processed by telecheckftf (invalid data)",
...
foreach $username ( sort @userarray ) {
  if ( -e "/home/pay1/batchfiles/$devprod/stopgenfiles.txt" ) {
    unlink "/home/pay1/batchfiles/$devprod/telecheckftf/batchfile.txt";
    last;
  }
  ...
  &procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/$devprod/telecheckftf", "batchfile.txt", "write", "", $batchfilestr );
  ...
  &procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/devlogs/telecheckftf", "miscdebug.txt", "append", "misc", $printstr );
  $logfilestr .= "$username\n";
  &procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/$devprod/telecheckftf/$fileyear", "$username$time$pid.txt", "append", "", $logfilestr );
...
dbEOM
  ...
  for ( my $vali = 0 ; $vali < scalar(@sthtransvalarray) ; $vali = $vali + 17 ) {
    ...
    $chkstatus = substr( $auth_code, 50, 1 );
    if ( ( $operation eq "postauth" ) && ( $chkstatus eq "0" ) ) {    # paper deposit does not postauth
      next;
    }
    ...
    $enccardnumber = &smpsutils::getcardnumber( $username, $orderid, "telecheckftf", $enccardnumber );
...
dbEOM
    ...
    ( $status, $invoicenum, $response ) = &procutils::sendprocmsg( "$processid", "telecheckftf", "$username", "$orderid", "$message" );
    ...
    if ( $status ne "success" ) {
      ...
      print MAILERR "To: cprice\@plugnpay.com\n";
      print MAILERR "From: dcprice\@plugnpay.com\n";
      print MAILERR "Subject: telecheckftf - genfiles error\n";
      print MAILERR "\n";
  ...
  $checkinstr = "";
  $checkinstr .= "$username\n";
  &procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/$devprod/telecheckftf", "genfiles.txt", "append", "", $checkinstr );
}

if ( !-e "/home/pay1/batchfiles/$devprod/stopgenfiles.txt" ) {
  umask 0033;
  $checkinstr = "";
  &procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/$devprod/telecheckftf", "genfiles.txt", "write", "", $checkinstr );
}
...
sub endbatch {
  ...
  if ( $response =~ /^....[A-Z]/ ) {
    ...
    my $myindex   = 0;
    my %dataarray = ();
    my $pairstr   = substr( $response, 4 );
    my (@pairs) = split( /\x1c/, $pairstr );
    $errmsg = $pairs[0];
    $pairs[0] = "";
...
dbEOM
    ...
    #print MAILERR "To: cprice\@plugnpay.com\n";
    #print MAILERR "From: dcprice\@plugnpay.com\n";
    #print MAILERR "Subject: telecheckftf - FORMAT ERROR\n";
    #print MAILERR "\n";
    #print MAILERR "username: $username\n";
    ...
    print MAILERR "To: cprice\@plugnpay.com\n";
    print MAILERR "From: dcprice\@plugnpay.com\n";
    print MAILERR "Subject: telecheckftf - unkown error\n";
    print MAILERR "\n";
    print MAILERR "username: $username\n";
...
dbEOM
  ...
  my $dbquerystr = <<"dbEOM";
          update telecheckftf set batchnum=?
          where username=?
dbEOM
...
sub batchdetail {
  ...
  $bd[0] = 'B-';                                                # packet type identifier (2a)
  $bd[1] = $merchant_id . "\|";                                 # telecheckftf merchant id (8n)
  $bd[2] = 'L' . "PlugPayECA 20111028 WIN7 SNNK 031" . "\|";    # version control (67a)
  $bd[3] = '*G' . "$refnumber" . "\|";                          # telecheckftf trace id (22a)

  $tcode = "A";                                                 # A = accepted
  if ( $operation eq "return" ) {
    $tcode = "R";
    $bd[4] = ')' . "$tcode" . "\|";                             # transaction type (1a)
    $bd[5] = '*U' . "\|";                                       # total check amount (9n)
  } else {
    $bd[4] = 'q' . "$tcode" . "\|";                             # transaction type (1a)
  }

  $bd[6] = 'w' . "$orderid" . "\|";                             # merchant trace id (50a)
  my $amount = substr( $amount, 4 );
  $bd[7] = '0' . "$amount" . "\|";                              # total check amount (9n)

  if ( $operation ne "return" ) {
    my $datetime = substr( $auth_code, 28, 12 );
    $bd[8] = '*E' . "$datetime" . "\|";                         # merchant date time MMDDYYYYHHMM (12n)
  }

  $bd[9] = 's' . "$orderid" . "\|";                             # echo data (50a)
