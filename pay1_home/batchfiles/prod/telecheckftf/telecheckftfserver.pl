...
} elsif ( ( -e "/home/pay1/batchfiles/$devprod/telecheckftf/secondary.txt" ) && ( $ipaddress ne $secondaryipaddress ) ) {
  $mytime     = gmtime( time() );
  $logfilestr = "";
  $logfilestr .= "$mytime switching to secondary socket\n";
  &procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/$devprod/telecheckftf", "serverlogmsg.txt", "append", "", $logfilestr );
  $ipaddress = $secondaryipaddress;
  $port      = $secondaryport;
} elsif ( !( -e "/home/pay1/batchfiles/$devprod/telecheckftf/secondary.txt" ) && ( $ipaddress ne $primaryipaddress ) ) {
  $mytime     = gmtime( time() );
  $logfilestr = "";
...
$outfilestr = "";
$outfilestr .= "$temptime\n";
&procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/$devprod/telecheckftf", "accesstime.txt", "write", "", $outfilestr );
...
while (1) {
  ...
  if ( ( -e "/home/pay1/batchfiles/$devprod/telecheckftf/stopserver.txt" ) || ( -e "/home/pay1/stopfiles/stop_processors" ) ) {
  ...
  if ( $keepalivecnt >= 60 ) {
    ...
    #    open(logfile,">>/home/pay1/batchfiles/$devprod/telecheckftf/serverlogmsg.txt");
...
sub check {
  ...
  foreach $key ( keys %writearray ) {
    if ( $writearray{$key} < $ttime1 ) {
      ...
      #open(tempfile,">>/home/pay1/batchfiles/$devprod/telecheckftf/temp.txt");
  ...
  if ( $numtrans == 4 ) {
    ...
    &procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/$devprod/telecheckftf", "time.txt", "append", "", $tempfilestr );
  }
...
dbEOM
  ...
  for ( my $vali = 0 ; $vali < scalar(@sth1valarray) ; $vali = $vali + 5 ) {
    ...
    #while (1) {}
    my $printstr = "msgrcv: $trans_time $processid $username $orderid $message\n";
    &procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/devlogs/telecheckftf", "miscdebug.txt", "append", "misc", $printstr );
...
dbEOM
    ...
    my $delta  = $now - $mytime;
    if ( $delta > 60 ) {
      &procutils::updateprocmsg( $processid, "telecheckftf", "failure", "", "failure: message timeout" );
...
sub socketopen {
  ...
  $mytime     = gmtime( time() );
  $logfilestr = "";
  $logfilestr .= "\n$mytime socketopen attempt $addr $port\n";
  &procutils::filewrite( "$username", "telecheckftf", "/home/pay1/batchfiles/$devprod/telecheckftf", "serverlogmsg.txt", "append", "", $logfilestr );
...
sub socketwrite {
  ...
  #}
  #if (!SOCK) {
  #  open(outfile,">/home/pay1/batchfiles/$devprod/telecheckftf/temp.txt");
  #  print outfile "dddd\n";
  #  close(outfile);
  #  exit;
  #}
  ...
  if ( $checkmessage =~ /\]3([0-9a-zA-Z\- ]+)\[/ ) {
    $xs     = $1;
    $oldnum = $1;
    $xs =~ s/[0-9a-zA-Z]/x/g;
    $checkmessage =~ s/$oldnum/$xs/;
  }
  $temptime = gmtime( time() );
  if ( $message !~ /0004ECHO/ ) {
    $logfilestr .= "$username  $orderid  $ipaddress  $port\n";
...
sub socketread {
  ...
  while ( $count && select( $rout = $rin, undef, undef, $delay ) ) {
    ...
    #print "recv: $rlen tt$response" . "tt\n";
    #print "$mlen $rlen\n";
...
sub update {
  ...
  #print logfile "recv: $checkmessage\n";
  ...
  &procutils::updateprocmsg( $processid, "telecheckftf", "success", "$rsequencenum", "$response" );
