...
foreach $username ( sort @userarray ) {
  ...
  if ( $username =~ /^(dbainsomni50|insomniaco76|insomniaco83|dbainsomni53|dbainsomni79|dbainsomni68|dbainsomni87|dbainsomni90)$/ ) {
    open( MAILERR, "| /usr/lib/sendmail -t" );
    print MAILERR "To: cprice\@plugnpay.com\n";
    print MAILERR "From: dcprice\@plugnpay.com\n";
    print MAILERR "Subject: mercury - genfilesfix insomni\n";
    print MAILERR "\n";
    print MAILERR "postauth/return done on old mercury account\n";
  ...
  my $sth = $dbh->prepare(
    qq{
        select merchant_id,pubsecret,proc_type,status,features,processor
        from customers
  ...
  ( $merchant_id, $terminal_id, $proc_type, $status, $features, $processor ) = $sth->fetchrow;
  $sth->finish;

  my $sth = $dbh->prepare(
    qq{
        select bankid,industrycode,capabilities,batchtime,acctfilenum
        from mercury
        where username='$username'
        }
    )
  ...
  ( $bankid, $industrycode, $capabilities, $batchgroup, $acctfilenum ) = $sth->fetchrow;
  $sth->finish;
  ...
  if ( $processor ne "mercury" ) {
    $merchant_id = $acctfilenum;
  } else {
    next;
  }
