...
if ( ( $now >= $time6am ) && ( $now < $time7am ) ) {
  ...
  &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/devlogs/paytechtampa", "miscdebug.txt", "append", "misc", $printstr );
  exit;
}
...
$cnt = `ps -ef | grep -v grep | grep -v vim | grep perl | grep -c 'paytechtampa/genfiles.pl $mygroup'`;
if ( $cnt > 1 ) {
  ...
  print MAILERR "Subject: paytechtampa - genfiles $mygroup already running\n";
  print MAILERR "\n";
...
$outfilestr .= "$pidline\n";
&procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "pid$mygroup.txt", "write", "", $outfilestr );

&miscutils::mysleep(2.0);

my $chkline = &procutils::fileread( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "pid$mygroup.txt" );
chop $chkline;
...
if ( $pidline ne $chkline ) {
  ...
  print MAILERR "From: dcprice\@plugnpay.com\n";
  print MAILERR "Subject: paytechtampa - dup genfiles\n";
  print MAILERR "\n";
...
my $checkuser = &procutils::flagread( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "genfiles$mygroup.txt" );
chop $checkuser;
...
if ( !-e "/home/pay1/batchfiles/$devprod/paytechtampa/$fileyearonly" ) {
  my $printstr = "creating $fileyearonly\n";
  &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/devlogs/paytechtampa", "miscdebug.txt", "append", "misc", $printstr );
  system("mkdir /home/pay1/batchfiles/$devprod/paytechtampa/$fileyearonly");
  chmod( 0700, "/home/pay1/batchfiles/$devprod/paytechtampa/$fileyearonly" );
}
if ( !-e "/home/pay1/batchfiles/$devprod/paytechtampa/$filemonth" ) {
  my $printstr = "creating $filemonth\n";
  &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/devlogs/paytechtampa", "miscdebug.txt", "append", "misc", $printstr );
  system("mkdir /home/pay1/batchfiles/$devprod/paytechtampa/$filemonth");
  chmod( 0700, "/home/pay1/batchfiles/$devprod/paytechtampa/$filemonth" );
}
if ( !-e "/home/pay1/batchfiles/$devprod/paytechtampa/$fileyear" ) {
  my $printstr = "creating $fileyear\n";
  &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/devlogs/paytechtampa", "miscdebug.txt", "append", "misc", $printstr );
  system("mkdir /home/pay1/batchfiles/$devprod/paytechtampa/$fileyear");
  chmod( 0700, "/home/pay1/batchfiles/$devprod/paytechtampa/$fileyear" );
}
if ( !-e "/home/pay1/batchfiles/$devprod/paytechtampa/$fileyear" ) {
  open( MAILERR, "| /usr/lib/sendmail -t" );
  print MAILERR "To: cprice\@plugnpay.com\n";
  print MAILERR "From: dcprice\@plugnpay.com\n";
  print MAILERR "Subject: paytechtampa - FAILURE\n";
  print MAILERR "\n";
  print MAILERR "Couldn't create directory logs/paytechtampa/$fileyear.\n\n";
  close MAILERR;
  exit;
}
...
foreach $username ( sort @userarray ) {
  if ( -e "/home/pay1/batchfiles/$devprod/stopgenfiles.txt" ) {
    ...
    &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa/$fileyear", "$username$time$pid.txt", "append", "", $logfilestr );

    unlink "/home/pay1/batchfiles/$devprod/paytechtampa/batchfile.txt";
    last;
  }
  ...
  $checkinstr = "";
  $checkinstr .= "$username\n";
  &procutils::flagwrite( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "genfiles$mygroup.txt", "write", "", $checkinstr );

  umask 0033;
  $batchfilestr = "";
  $batchfilestr .= "$username\n";
  &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "batchfile.txt", "write", "", $batchfilestr );
...
dbEOM
    ...
    $enccardnumber = &smpsutils::getcardnumber( $username, $orderid, "paytechtampa", $enccardnumber );
...
dbEOM
        ...
        &procutils::dbinsert( $username, $orderid, "pnpmisc", "paytechtampa", %inserthash );

      } else {
        my $dbquerystr = <<"dbEOM";
              update paytechtampa set batchnum=?
              where username=?
dbEOM
...
sub mysleep {
  for ( $myi = 0 ; $myi <= 60 ; $myi++ ) {
    ...
    $outfilestr = "";
    $outfilestr .= "$temptime\n";
    &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "baccesstime.txt", "write", "", $outfilestr );
...
sub senderrmail {
  ...
  print MAILERR "To: cprice\@plugnpay.com\n";
  print MAILERR "From: dcprice\@plugnpay.com\n";
  print MAILERR "Subject: paytechtampa - batch problem\n";
  print MAILERR "\n";
...
dbEOM
  ...
  if ( ( $group eq "trailer" ) && ( $respcode eq "00" ) ) {
    ...
    foreach $oid (@orderidarray) {
      my $printstr = "$username $oid success\n";
      &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/devlogs/paytechtampa", "miscdebug.txt", "append", "misc", $printstr );
      my $dbquerystr = <<"dbEOM";
...
dbEOM
    ...
    foreach $oid (@orderidarray) {
      my $printstr = "$username $oid pending\n";
      &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/devlogs/paytechtampa", "miscdebug.txt", "append", "misc", $printstr );
      my $dbquerystr = <<"dbEOM";
...
sub batchdetail {
  ...
  if ( ( $refnumber eq "" ) || ( $refnumber eq "000000000000" ) ) {
    $refnum = &smpsutils::gettransid( $username, "paytechtampa", $orderid );
  ...
  if ( $datainfo{'transflags'} =~ /gift/ ) {
    ...
    &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "gift.txt", "append", "", $tmpfilestr );
  }
  ...
  my $printstr = "dddd dddd $transflags\n";
  &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/devlogs/paytechtampa", "miscdebug.txt", "append", "misc", $printstr );
