...
if ( -e "/home/pay1/batchfiles/$devprod/paytechtampa/failover.txt" ) {
  exit;
}
...
while (1) {
  ...
  my $outfilestr = "";
  $outfilestr .= "$temptime\n";
  &procutils::filewrite( "ptechserver", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "ptech.txt", "write", "", $outfilestr );

  if ( ( -e "/home/pay1/batchfiles/$devprod/paytechtampa/stopserver.txt" ) || ( -e "/home/pay1/stopfiles/stop_processors" ) ) {
    close(SOCK);
    sleep 1;
    exit;
  }
  ...
  if ( $getrespflag == 0 ) {
    ...
    &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "serverlogmsg.txt", "append", "", $logfilestr );
...
sub check {
  ...
  foreach $key ( keys %writearray ) {
    if ( $writearray{$key} < $ttime1 ) {
      ...
      &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "temp.txt", "append", "", $tempfilestr );
    }
  }
  ...
  if ( $numtrans == 4 ) {
    ...
    &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "time.txt", "append", "", $tempfilestr );
  }
...
dbEOM
    ...
    my $delta  = $now - $mytime;
    if ( $delta > 60 ) {
      &procutils::updateprocmsg( $processid, "paytechtampa", "failure", "", "failure: message timeout" );
    ...
    $outfilestr = "";
    $outfilestr .= "$temptime\n";
    &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "ptech.txt", "write", "", $outfilestr );
...
sub socketwrite {
  ...
  if ( $socketopenflag != 1 ) {
    ...
    &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "serverlogmsg.txt", "write", "", $outfilestr );
    exit;
  }
      ...
      if ( $nullresp !~ /PTIISOYN          1810/ ) {
        ...
        if ( !%writearray ) {
          ...
          foreach $key ( keys %writearray ) {
            ...
            &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "serverlogmsg.txt", "append", "", $tempfilestr );
          }
        }
      } else {
        $getrespflag = 1;
...
sub updatepaytech {
  ...
  &procutils::updateprocmsg( $processid, "paytechtampa", "success", "$sinvoicenum{$rsequencenum}", "$response" );
...
sub checksecondary {
  my @tmpfilestrarray = &procutils::fileread( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "secondary.txt" );
  ...
  if ( $test eq "yes" ) {
    ...
    $host      = $testhost;
  } elsif ( ( -e "/home/pay1/batchfiles/$devprod/paytechtampa/secondary.txt" ) && ( $ipaddress ne $secondaryipaddress ) ) {
    $tmpfilestr = "";
    $tmpfilestr .= "secondary $secondary\n";
    &procutils::filewrite( "$username", "paytechtampa", "/home/pay1/batchfiles/$devprod/paytechtampa", "serverlogmsg.txt", "append", "", $tmpfilestr );
