...
} elsif ( ( -e "/home/pay1/batchfiles/$devprod/fdmsrc/secondary.txt" ) && ( $ipaddress ne $secondaryipaddress ) ) {
  $mytime     = gmtime( time() );
  $logfilestr = "";
  $logfilestr .= "$mytime switching to secondary socket\n";
  $logfilestr .= "$sockaddrport\n";
  $logfilestr .= "$sockettmp\n\n";
  &procutils::filewrite( "$username", "fdmsrc", "/home/pay1/batchfiles/$devprod/fdmsrc", "serverlogmsg.txt", "append", "", $logfilestr );
  $ipaddress = $secondaryipaddress;
  $port      = $secondaryport;
} elsif ( !( -e "/home/pay1/batchfiles/$devprod/fdmsrc/secondary.txt" ) && ( $ipaddress ne $primaryipaddress ) ) {
  $mytime     = gmtime( time() );
  $logfilestr = "";
...
while (1) {
  ...
  $outfilestr = "";
  $outfilestr .= "$temptime\n";
  &procutils::filewrite( "$username", "fdmsrc", "/home/pay1/batchfiles/$devprod/fdmsrc", "accesstime.txt", "write", "", $outfilestr );
...
dbEOM
  ...
  for ( my $vali = 0 ; $vali < scalar(@sth1valarray) ; $vali = $vali + 7 ) {
    ...
    if ( $processormsgstatus eq "success" ) {
      ...
      my $delta  = $now - $mytime;
      if ( $delta > 65 ) {    # we use 65 normally
                              #my ($enclen,$encdata) = split(/ /,$message,2);
                              #$message = &rsautils::rsa_decrypt_file($encdata,$enclen,"print enccardnumber 497","/home/pay1/pwfiles/keys/key");
        ...
        if ( $message =~ /<(Credit|Debit)Request/ ) {
          ...
          if ( ( $paymenttype =~ /^(Credit|Debit)$/ ) && ( $respcode =~ /^(000|002|85)$/ ) ) {
            ...
            $checkmessage = substr( $message, 2 );
            $checkmessage = &fdmsrc::stripfields($checkmessage);
...
dbEOM
          ...
          &procutils::dbinsert( $username, $orderid, "pnpmisc", "fdmsrc", %inserthash );

        } else {
          my $dbquerystr = <<"dbEOM";
                update fdmsrc set invoicenum=?
                where username=?
dbEOM
...
sub updatefdmsrc {
  ...
  # for testing voidmessage where there is also no void response
...
dbEOM
  ...
  if ( ( $mtrsequencenum ne "" ) && ( $mid eq $smid{"$mtrsequencenum"} ) && ( $tid eq $stid{"$mtrsequencenum"} ) ) {
    ...
    &procutils::filewrite( "$username", "fdmsrc", "/home/pay1/batchfiles/$devprod/fdmsrc", "bbserverlogmsg.txt", "append", "", $logfilestr );
...
sub socketclose {
  ...
  print MAIL "Subject: fdmsrc - no response to authorization\n";
  print MAIL "\n";

  print MAIL "fdmsrc socket is being closed, then reopened because no response was\n\n";
...
sub mysqlmsgsnd {
  ...
  if ( length($encdata) > 3600 ) {
    ...
    print MAIL "To: cprice\@plugnpay.com\n";
    print MAIL "From: dprice\@plugnpay.com\n";
    print MAIL "Subject: fdmsrc - response too large\n";
    print MAIL "\n";

    my $encdatalen = length($encdata);

    print MAIL "fdmsrc response is too large for processormsg.  $encdatalen\n\n";
