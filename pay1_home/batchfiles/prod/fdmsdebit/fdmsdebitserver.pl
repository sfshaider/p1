...
} elsif ( ( -e "/home/pay1/batchfiles/$devprod/fdmsdebit/secondary.txt" ) && ( $ipaddress ne $secondaryipaddress ) ) {
  $mytime     = gmtime( time() );
  $logfilestr = "";
  $logfilestr .= "$mytime switching to secondary socket\n";
  $logfilestr .= "$sockaddrport\n";
  $logfilestr .= "$sockettmp\n\n";
  &procutils::filewrite( "$username", "fdmsdebit", "/home/pay1/batchfiles/$devprod/fdmsdebit", "serverlogmsg.txt", "append", "", $logfilestr );
  $ipaddress = $secondaryipaddress;
  $port      = $secondaryport;
} elsif ( !( -e "/home/pay1/batchfiles/$devprod/fdmsdebit/secondary.txt" ) && ( $ipaddress ne $primaryipaddress ) ) {
  $mytime     = gmtime( time() );
  $logfilestr = "";
...
while (1) {
  ...
  $outfilestr = "";
  $outfilestr .= "$temptime\n";
  &procutils::filewrite( "$username", "fdmsdebit", "/home/pay1/batchfiles/$devprod/fdmsdebit", "accesstime.txt", "write", "", $outfilestr );
      ...
      if ( $delta > 65 ) {
        ...
        if ( $message =~ /<(Credit|Debit)Request/ ) {
          ...
          if ( ( $paymenttype =~ /^(Credit|Debit)$/ ) && ( $respcode =~ /^(000|002|85)$/ ) ) {
            ...
            $checkmessage = &fdmsdebit::stripfields($checkmessage);
...
dbEOM
    ...
    my $delta  = $now - $mytime;
    if ( $delta > 60 ) {
      &procutils::updateprocmsg( $processid, "fdmsdebit", "failure", "", "failure: message timeout" );
      next;
    }
...
dbEOM
      ...
      if ( $message eq "invoicenum" ) {
        ...
        &procutils::updateprocmsg( $processid, "fdmsdebit", "success", "$invoicenum", "$invoicenum" );
        next;
      }
...
sub updatefdmsdebit {
  my $printstr = "in updatefdmsdebit\n";
  &procutils::filewrite( "$username", "fdmsdebit", "/home/pay1/batchfiles/devlogs/fdmsdebit", "miscdebug.txt", "append", "misc", $printstr );
...
dbEOM
  ...
  if ( ( $mtrsequencenum ne "" ) && ( $mid eq $smid{"$mtrsequencenum"} ) && ( $tid eq $stid{"$mtrsequencenum"} ) ) {
    ...
    &procutils::updateprocmsg( $processid, "fdmsdebit", "success", "", "$response" );
...
sub socketclose {
  ...
  print MAIL "Subject: fdmsdebit - no response to authorization\n";
  print MAIL "\n";

  print MAIL "fdmsdebit socket is being closed, then reopened because no response was\n\n";
...
sub voidmessage {
  ...
  my $checkmessage = &fdmsdebit::stripfields($message);
