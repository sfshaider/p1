...
if ( $test eq "yes" ) {
  ...
  &procutils::filewrite( "$username", "fdms", "/home/pay1/batchfiles/$devprod/fdms", "serverlogmsg.txt", "append", "", $logfilestr );
...
while (1) {
  ...
  $outfilestr = "";
  $outfilestr .= "$temptime\n";
  &procutils::filewrite( "$username", "fdms", "/home/pay1/batchfiles/$devprod/fdms", "accesstime.txt", "write", "", $outfilestr );
...
dbEOM
    ...
    my $delta  = $now - $mytime;
    if ( $delta > 60 ) {
      &procutils::updateprocmsg( $processid, "fdms", "failure", "", "failure: message timeout" );
    ...
    if ( $message ne "invoicenum" ) {
      ...
      my $subseq = substr( $sequencenum, -5, 5 );
      $message = substr( $message, 0, 52 ) . $subseq . substr( $message, 57 );
    }
...
dbEOM
    ...
    if ( $message eq "invoicenum" ) {
      ...
      &procutils::updateprocmsg( $processid, "fdms", "success", "$invoicenum", "invoicenum" );
...
sub updatefdms {
  ...
  &procutils::updateprocmsg( $processid, "fdms", "success", "$sinvoicenum{$rsequencenum}", "$response" );
