...
foreach my $userkey ( sort keys %userlist ) {
  ...
  print "select username,enccardnumber,length,exp from customers where status=live\n";

  #limit 20
  my $dbquerystr = <<"dbEOM";
        select username,enccardnumber,length,exp_date
        from customers
	where status='live'
dbEOM
  my @dbvalues = ();
  ...
  for ( my $vali = 0 ; $vali < scalar(@sthvalarray) ; $vali = $vali + 4 ) {
    ...
    $enccardnumber = &smpsutils::getcardnumber( "pnpbilling", $username, "customers", $enccardnumber, "rec" );

    if ( $enccardnumber ne "" ) {
      $cardnumber = &rsautils::rsa_decrypt_file( $enccardnumber, $length, "print enccardnumber 497", "/home/pay1/pwfiles/keys/key" );

      #if ($cardnumber =~ /[^0-9\ ]/) {  ### Check for characters other than numbers and a space.  If found try to decrypt using old key
      #  $cardnumber = &rsautils::rsa_decrypt_file($enccardnumber,$length,"$passphrase","/home/pay1/web/private/key");
      #}
    } else {
      next;
    }
