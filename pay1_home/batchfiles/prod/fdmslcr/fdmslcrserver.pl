...
} elsif ( ( -e "/home/pay1/batchfiles/$devprod/fdmslcr/secondary.txt" ) && ( $ipaddress ne $secondaryipaddress ) ) {
  $mytime     = gmtime( time() );
  $logfilestr = "";
  $logfilestr .= "$mytime switching to secondary socket\n";
  $logfilestr .= "$sockaddrport\n";
  $logfilestr .= "$sockettmp\n\n";
  &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr", "serverlogmsg.txt", "append", "", $logfilestr );
  $ipaddress = $secondaryipaddress;
  $port      = $secondaryport;
} elsif ( !( -e "/home/pay1/batchfiles/$devprod/fdmslcr/secondary.txt" ) && ( $ipaddress ne $primaryipaddress ) ) {
  $mytime     = gmtime( time() );
  $logfilestr = "";
...
while (1) {
  ...
  $outfilestr = "";
  $outfilestr .= "$temptime\n";
  &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr", "accesstime.txt", "write", "", $outfilestr );
    ...
    if ( $processormsgstatus eq "success" ) {
      ...
      if ( $delta > 65 ) {
        if ( $message =~ /<(Credit|Debit)Request/ ) {
          ...
          $printstr .= "paymenttype: $paymenttype\n";
          &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/devlogs/fdmslcr", "miscdebug.txt", "append", "misc", $printstr );
          ...
          if ( ( $paymenttype =~ /^(Credit|Debit)$/ ) && ( $respcode =~ /^(000|002|85)$/ ) ) {
            ...
            $checkmessage = &fdmslcr::stripfields($checkmessage);
...
dbEOM
    ...
    my $delta  = $now - $mytime;
    if ( $delta > 60 ) {
      &procutils::updateprocmsg( $processid, "fdmslcr", "failure", "", "failure: message timeout" );
      next;
    }
    ...
    if ( ( $message ne "invoicenum" ) && ( $temparray{"GMF,$requesttype,CommonGrp,STAN"} ne "000000" ) && ( $temparray{"GMF,$requesttype,CommonGrp,STAN"} ne "" ) ) {
...
dbEOM
      ...
      if ( $message eq "invoicenum" ) {
        my $printstr = "message  $message   response  $invoicenum\n";
        &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/devlogs/fdmslcr", "miscdebug.txt", "append", "misc", $printstr );

        &procutils::updateprocmsg( $processid, "fdmslcr", "success", "$invoicenum", "$invoicenum" );
        next;
      }
      ...
      $ordernum = $temparray{"GMF,$requesttype,CommonGrp,OrderNum"};    # bit 37
      if ( ( $ordernum eq "00000000" ) || ( $ordernum eq "" ) ) {
    ...
    if ( $sstatus{"$rsequencenum"} ne "done" ) {
      ...
      if ( ( $delta > 240 ) && ( ( $smessage{"$rsequencenum"} !~ /<(Credit|Debit)Request/ ) || ( $scardtype{"$rsequencenum"} eq "interac" ) ) ) {
        ...
        if ( ( $delta < 180 ) && ( $tmpstr =~ /($chkstr1|$chkstr2|$chkstr3)/ ) ) {    # void all messages
          if (
            (0)
            && ( ( ( $delta > 40 ) && ( $sretries{"$rsequencenum"} < 2 ) )
...
sub socketopen {
  ...
  connect( SOCK, $paddr ) || &socketopen2( "206.201.50.177", 22835, "connect: $!" );    # for testing only
                                                                                        #&socketopen2("206.201.50.177",22835,"connect: $!");	# for testing only
  $socketopenflag = 1;
...
sub updatefdmslcr {
  ...
  # GMF,RejectResponse,GMF,ReversalRequest,CommonGrp,RefNum    bb 000001034330
...
dbEOM
  ...
  foreach my $seqkey ( sort keys %sorderid ) {
    $printstr .= "$seqkey    $smid{$seqkey}    $stid{$seqkey}\n";
  }
  &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/devlogs/fdmslcr", "miscdebug.txt", "append", "misc", $printstr );
  ...
  if ( ( $mtrsequencenum ne "" ) && ( $mid eq $smid{"$mtrsequencenum"} ) && ( $tid eq $stid{"$mtrsequencenum"} ) ) {
    ...
    &procutils::updateprocmsg( $processid, "fdmslcr", "success", "", "$response" );
...
sub socketclose {
  ...
  print MAIL "Subject: fdmslcr - no response to authorization\n";
  print MAIL "\n";

  print MAIL "fdmslcr socket is being closed, then reopened because no response was\n\n";
...
sub voidmessage {
  ...
  my $checkmessage = &fdmslcr::stripfields($message);
  my $printstr     = "origmessage: $checkmessage\n";
  &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/devlogs/fdmslcr", "miscdebug.txt", "append", "misc", $printstr );
...
dbEOM
  ...
  $printstr .= "mmmm $mtsequencenum\n";
  my $tmpstr = substr( $smessage{"$mtsequencenum"}, 0, 80 );
  $printstr .= "nnnn $tmpstr\n";
  &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/devlogs/fdmslcr", "miscdebug.txt", "append", "misc", $printstr );
