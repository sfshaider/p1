...
# mailbox name NAGW-SCHHC001
# Job name RCDMPNPS   for uploading files
# Job name MRDXMPNP   for downloading files

# sftp -oIdentityFile=.ssh/id_rsa -oPort=6522 NAGW-SCHHC001@test-gw-na.firstdataclients.com     # test
# sftp -oIdentityFile=.ssh/id_rsa -oPort=6522 NAGW-SCHHC001@test2-gw-na.firstdataclients.com    # test
# sftp -oIdentityFile=.ssh/id_rsa -oPort=6522 NAGW-SCHHC001@prod-gw-na.firstdataclients.com     # prod
# sftp -oIdentityFile=.ssh/id_rsa -oPort=6522 NAGW-SCHHC001@prod2-gw-na.firstdataclients.com    # prod
#### sftp -oIdentityFile=.ssh/id_rsa -oPort=6522 NAGW-SCHHC001@prod2-gw-na.firstdataclients.com # production

$cnt = `ps -ef | grep -v grep | grep -v vim | grep perl | grep -c 'fdmslcr/getfiles.pl'`;
if ( $cnt > 1 ) {
  my $printstr = "get.pl already running, exiting...\n";
  &procutils::filewrite( "getfiles", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr", "ftplog.txt", "append", "misc", $printstr );
  exit;
}
...
if ( !-e "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyearonly" ) {
  my $printstr = "creating $fileyearonly\n";
  &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr", "ftplog.txt", "append", "misc", $printstr );
  system("mkdir /home/pay1/batchfiles/$devprod/fdmslcr/$fileyearonly");
  chmod( 0700, "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyearonly" );
}
if ( !-e "/home/pay1/batchfiles/$devprod/fdmslcr/$filemonth" ) {
  my $printstr = "creating $filemonth\n";
  &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr", "ftplog.txt", "append", "misc", $printstr );
  system("mkdir /home/pay1/batchfiles/$devprod/fdmslcr/$filemonth");
  chmod( 0700, "/home/pay1/batchfiles/$devprod/fdmslcr/$filemonth" );
}
if ( !-e "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyear" ) {
  my $printstr = "creating $fileyear\n";
  &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr", "ftplog.txt", "append", "misc", $printstr );
  system("mkdir /home/pay1/batchfiles/$devprod/fdmslcr/$fileyear");
  chmod( 0700, "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyear" );
}
if ( !-e "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyear" ) {
  ...
  print MAILERR "\n";
  print MAILERR "Couldn't create directory logs/fdmslcr/$fileyear.\n\n";
  close MAILERR;
  exit;
}
...
dbEOM
  ...
  if ( ( $chkfilename ne "" ) && ( ( $todayhour - $filenamehour ) > 1 ) ) {
    ...
    print MAILERR "To: cprice\@plugnpay.com\n";
    print MAILERR "From: dcprice\@plugnpay.com\n";
    print MAILERR "Subject: fdmslcr - getfiles FAILURE\n";
    print MAILERR "\n";
...
my %args = (
  ...
  password => 'K)vZ.6R\'J',
  port     => 6522,
  key_path => '/home/pay1/batchfiles/prod/fdmslcr/.ssh/id_rsa'
);
...
foreach $var (@$files) {
  ...
  if ( ( $filename =~ /\.out/ ) && ( !-e "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyear/$filename.done" ) ) {
    my $printstr = "filename: $filename\n";
    $printstr .= "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyear/$filename\n";
    &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr", "ftplog.txt", "append", "misc", $printstr );

    $ftp->get( "available/$filename", "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyear/$filename", copy_perm => 0, copy_time => 0 ) or die "file transfer failed: " . $ftp->error;
...
dbEOM
  ...
  my %failarray      = ();
  my $infilestr      = &procutils::fileread( "$username", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyear", "$filename" );
  ...
  foreach (@infilestrarray) {
    ...
    if ( $line =~ /^S/ ) {
      ...
      if ( length($detailnum) == 12 ) {
        $detailnum =~ s/^0+//g;    # detailnum maps to orderid, orderids less than 12 length are padded with 0s, need to remove the 0s
      }
...
dbEOM
      ...
      if ( $depositflag eq "N" ) {
        ...
        print MAILERR "To: cprice\@plugnpay.com\n";
        print MAILERR "From: dcprice\@plugnpay.com\n";
        print MAILERR "Subject: fdmslcr - $username FAILURE\n";
        print MAILERR "\n";
...
dbEOM
  ...
  &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyear", "$filename.done", "write", "", $outfilestr );

  unlink "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyear/$filename";
}
