...
if ( ( -e "/home/pay1/batchfiles/stopgenfiles.txt" ) || ( -e "/home/pay1/batchfiles/$devprod/fdmslcr/stopgenfiles.txt" ) ) {
  exit;
}

$cnt = `ps -ef | grep -v grep | grep -v vim | grep perl | grep -c 'fdmslcr/putfiles.pl'`;
if ( $cnt > 1 ) {
...
if ( !-e "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyear" ) {
  ...
  print MAILERR "\n";
  print MAILERR "Couldn't create directory $devprod/fdmslcr/$fileyear.\n\n";
  close MAILERR;
  exit;
}
...
# fdmslcr can only send one result at a time
my $dbquerystr = <<"dbEOM";
...
if ( $filecnt >= 3 ) {
  ...
  print MAILERR "To: cprice\@plugnpay.com\n";
  print MAILERR "From: dcprice\@plugnpay.com\n";
  print MAILERR "Subject: fdmslcr - putfiles FAILURE\n";
  print MAILERR "\n";
...
for ( my $vali = 0 ; $vali < scalar(@sth_batchvalarray) ; $vali = $vali + 1 ) {
  ...
  my $printstr = "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyear/$filename\n";
  &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr", "ftplog.txt", "append", "misc", $printstr );

  $lines  = `ls -l /home/pay1/batchfiles/$devprod/fdmslcr/$fileyear/$filename.txt`;
  $mytime = gmtime( time() );
  my $printstr = "$mytime bbbb $lines bbbb\n";
  &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr", "ftplog.txt", "append", "misc", $printstr );

  if ( -e "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyear/$filename.txt" ) {
    my $printstr = "before fileencread /home/pay1/batchfiles/$devprod/fdmslcr/$fileyear/$filename\n";
    &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr", "ftplog.txt", "append", "misc", $printstr );

    my $msg = &procutils::fileencread( "putfiles", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr/$fileyear", "$filename", "" );

    my $printstr = "after fileencread\n";
    &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr", "ftplog.txt", "append", "misc", $printstr );

    if ( length($msg) > 60 ) {
      my $printstr = "before put file   /home/pay1/batchfiles/$devprod/fdmslcr/$fileyear/$filename  /CI000489.$filename.txt\n";
      &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/$devprod/fdmslcr", "ftplog.txt", "append", "misc", $printstr );

      $ftp->put_content( "$msg", "/CI000489.$filename.txt" );
    ...
    $OUTFILEstr .= "filename: /CI000489.$filename.txt\n\n";
    ...
    $OUTFILEstr .= "mailbox:          NAGW\-SCHHC001\n";
    $OUTFILEstr .= "inbound job name: CI000489\n";
    $OUTFILEstr .= "time:             $savtime\n\n";
    &procutils::filewrite( "$username", "fdmslcr", "/home/pay1/batchfiles/logs/fdmslcr", "ftpissue.txt", "write", "", $OUTFILEstr );
