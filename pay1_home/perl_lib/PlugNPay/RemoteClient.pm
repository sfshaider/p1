...
sub setRemoteClientUsername {
  my $self = shift;
  my $remoteClientUsername = shift;
  $self->{'remoteClientUsername'} = $remoteClientUsername;
}

sub getRemoteClientUsername {
  my $self = shift;
  return $self->{'remoteClientUsername'};
}
...
sub _savePermittedNetworkAddresses {
  ...
  if (!$@) {
    ...
    die('Unable to save Remote Client { username: ' . $self->getGatewayAccount() . '}');
  }
}
...
sub _loadPermittedNetworkAddresses {
  ...
  $sth->execute($self->getGatewayAccount()) or die 'Unable to load permitted addresses ' . $DBI::errstr;
...
sub manageRemoteClientAccount {
  my $self = shift;
  my $password = shift;
  my $username = shift || $self->getRemoteClientUsername();
  ...
  if (!defined $username) {
    if (!defined $gatewayAccount) {
      ...
      $username = 'rc_' . $gatewayAccount;
    }
  } elsif ($username !~ /^rc_/) {
    $username = 'rc_' . $username;
  }
