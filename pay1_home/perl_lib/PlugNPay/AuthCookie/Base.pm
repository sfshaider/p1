package PlugNPay::AuthCookie::Base;
$PlugNPay::AuthCookie::Base::VERSION = '3.26';
...
use PlugNPay::AuthCookie::Params;
use Apache2::Access ();
...
sub cookie_string {
    ...
    if (my $expires = $p{expires} || $r->dir_config("${auth_name}Expires")) {
        $expires = PlugNPay::AuthCookie::Util::expires($expires);
...
sub login {
    ...
    my $destination = $redirector->filterRedirection($params->param('destination'));
    ...
    $r->headers_out->set("Location" => $self->untaint_destination($destination));
...
sub login_form_status {
    ...
    if (PlugNPay::AuthCookie::Util::understands_forbidden_response($ua)) {
...
sub params {
    ...
    return PlugNPay::AuthCookie::Params->new($r);
}
...
sub untaint_destination {
    ...
    return PlugNPay::AuthCookie::Util::escape_destination($dest);
}
...
PlugNPay::AuthCookie::Base - Common Methods Shared by Apache2 and Apache2_4 AuthCookie Subclasses.
...
=head2 params($r): PlugNPay::AuthCookie::Params
