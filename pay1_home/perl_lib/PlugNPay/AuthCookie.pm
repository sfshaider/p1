package PlugNPay::AuthCookie;
$PlugNPay::AuthCookie::VERSION = '3.26';
# ABSTRACT: Perl Authentication and Authorization via cookies

use strict;

use Carp;
use base 'PlugNPay::AuthCookie::Base';
...
use PlugNPay::AuthCookie::Util qw(is_blank);
...
PlugNPay::AuthCookie - Perl Authentication and Authorization via cookies
...
MethodHandlers, Authen, and Authz compiled in.
 ...
 PerlModule Sample::PlugNPay::AuthCookieHandler
 ...
 <Location /protected>
  AuthType Sample::PlugNPay::AuthCookieHandler
  AuthName WhatEver
  PerlAuthenHandler Sample::PlugNPay::AuthCookieHandler->authenticate
  PerlAuthzHandler Sample::PlugNPay::AuthCookieHandler->authorize
  require valid-user
 </Location>
 ...
 <FilesMatch "\.ok$">
  AuthType Sample::PlugNPay::AuthCookieHandler
  AuthName WhatEver
  PerlFixupHandler Sample::PlugNPay::AuthCookieHandler->recognize_user
 </FilesMatch>
 ...
 <Files LOGIN>
  ...
  PerlResponseHandler Sample::PlugNPay::AuthCookieHandler->login
 </Files>
...
B<PlugNPay::AuthCookie> allows you to intercept a user's first
...
C<PlugNPay::AuthCookie>.  Its job is to create the session key that will
...
You must define this method yourself in your subclass of
PlugNPay::AuthCookie.  Its job is to look at a session key and determine
...
For an example of how to use PlugNPay::AuthCookie, you may want to check
