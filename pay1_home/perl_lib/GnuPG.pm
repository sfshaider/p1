...
sub run_gnupg($) {
	...
	# Besides this is just plain good hygiene
        ### Modifed by DCP 20060525 added &POSIX:: and extra set of ()
        my $max_fd = POSIX::sysconf( &POSIX::_SC_OPEN_MAX() ) || 256;
	# orig my $max_fd = POSIX::sysconf( _SC_OPEN_MAX ) || 256;
...
sub import_keys($%) {
  ...
  KEY:
    ...
    ## DCP Modified  added  "|IMPORT_OK" to support new feature of GnuPGP
    # orig unless $cmd =~ /IMPORT_RES/;
    $self->abort_gnupg ( "protocol error expected IMPORT_RES got $cmd\n" )
      unless $cmd =~ /IMPORT_RES|IMPORT_OK/;
    $self->end_gnupg;
...
sub decrypt_postread($) {
    ...
    ## DCP Modifed to add '|PLAINTEXT'
    # orig       unless $cmd =~ /DECRYPTION_OKAY/;
    $self->abort_gnupg( "protocol error: expected DECRYPTION_OKAY got $cmd: \n" )
      unless $cmd =~ /DECRYPTION_OKAY|PLAINTEXT/;
